...
async function getModels(searchParams: SearchParams) {
  // First fetch bike brand IDs to filter at DB level
  const { data: bikeBrands } = await supabase
    .from('brands')
    .select('id, slug')
    .or('vehicle_type.eq.bike,vehicle_type.eq.both')
    .eq('is_active', true)

  const bikeBrandIds = (bikeBrands || []).map((b: any) => b.id)

  let query = supabase
    .from('models')
    .select(`
      id, name, slug, thumbnail_url, engine_cc, specs,
      brand:brands!inner(id, name, slug, vehicle_type)
    `)
    .eq('is_active', true)
    .in('brand_id', bikeBrandIds)

  if (searchParams.brand) {
    const matchBrand = (bikeBrands || []).find((b: any) => b.slug === searchParams.brand)
